...
- **存储安全**：密钥以**明文**存于 `data/api-keys.json`（含 `reveal` 接口可回看完整 Key），并非 hash 形式。该文件由服务以 `0600` 写入、目录 `0700`，仅服务用户可读；请务必保护好 `data/` 卷——限制文件系统权限、勿随镜像/仓库提交、勿对外暴露。
...
1. **严格客户端不再报错** —— 此前每一轮回答都会被判定为「响应被截断」并标记失败，现在能正常结束。
2. **「正常结束」与「上游故障」可区分** —— 此前两者在客户端看来完全一样（流突然没了），现在上游故障会作为一条明确的错误帧送达。
3. **工具调用语义正确** —— 此前工具调用无法被识别，现在返回 `finish_reason: "tool_calls"`，Agent 框架能据此触发下一轮工具执行。

> 说明：本次为协议级缺陷修复，未新增功能、未改动性能，非流式响应与 `/v1/responses`、`/v1/messages` 亦未改动。
...
- `sseFinishChunk()`：统一构造并发送上述终止帧，三处分支共用，避免协议细节散落在各分支里各写一遍。
- `sseErrorChunk()`：上游在流中途失败时向客户端发送错误帧，而不再静默关闭连接（原先「静默关闭」与「正常结束但没给结束原因」在客户端看来完全一样，无法区分故障与正常）。
